    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                usuario=request.user, descricao=f"Criou o Tipo de Leito '{form.cleaned_data['nome']}' de cor '{form.cleaned_data['cor']}'.")
    else:
        form = TipoDeLeitoForm()

    query = request.GET.get('q', '')
    if query:
        objs = TipoDeLeito.objects.filter(
...
def editar_tipo_de_leito_view(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                usuario=request.user, descricao=f"Editou o Tipo de Leito '{form.initial['nome']}' de cor '{form.initial['cor']}' para o Tipo de Leito '{form.cleaned_data['nome']}' de cor '{form.cleaned_data['cor']}'.")
...
def excluir_tipo_de_leito_view(request, id):
    ...
    if request.method == 'POST':
        ...
        Historico.objects.create(
            usuario=request.user, descricao=f"""Removeu o Tipo de Leito <span class="badge d-inline-flex align-items-center fw-bolder text-center" style="background-color: {obj.cor};">{obj.nome}</span>.""")

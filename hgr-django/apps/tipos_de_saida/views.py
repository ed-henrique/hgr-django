...
def tipos_de_saida_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                ...
                descricao=f"""Criou o Tipo de Saída <span class="badge d-inline-flex align-items-center fw-bolder text-center" style="background-color: {form.cleaned_data["cor"]};">{form.cleaned_data["nome"]}</span>.""",
            )
    else:
    ...
    if query:
        ...
        objs = TipoDeSaida.objects.filter(removido_em__isnull=True).order_by("nome")
...
def excluir_tipo_de_saida_view(request, id):
    ...
    if request.method == "POST":
        ...
        Historico.objects.create(
            usuario=request.user,
            descricao=f"""Removeu o Tipo de Saída <span class="badge d-inline-flex align-items-center fw-bolder text-center" style="background-color: {obj.cor};">{obj.nome}</span>.""",
        )

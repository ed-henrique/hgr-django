    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                usuario=request.user, descricao=f"Criou a Especialidade '{form.cleaned_data['nome']}' de cor '{form.cleaned_data['cor']}'.")
    else:
...
def editar_especialidade_view(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                usuario=request.user, descricao=f"Editou a Especialidade '{form.initial['nome']}' de cor '{form.initial['cor']}' para a Especialidade '{form.cleaned_data['nome']}' de cor '{form.cleaned_data['cor']}'.")
...
def excluir_especialidade_view(request, id):
    ...
    if request.method == 'POST':
        ...
        Historico.objects.create(
            usuario=request.user, descricao=f"""Removeu a Especialidade <span class="badge d-inline-flex align-items-center fw-bolder text-center" style="background-color: {obj.cor};">{obj.nome}</span>.""")

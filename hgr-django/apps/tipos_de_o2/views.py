    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                usuario=request.user, descricao=f"Criou o Tipo de O2 '{form.cleaned_data['nome']}' de cor '{form.cleaned_data['cor']}'.")
    else:
        form = TipoDeO2Form()
...
def editar_tipo_de_o2_view(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Historico.objects.create(
                usuario=request.user, descricao=f"Editou o Tipo de O2 '{form.initial['nome']}' de cor '{form.initial['cor']}' para o Tipo de O2 '{form.cleaned_data['nome']}' de cor '{form.cleaned_data['cor']}'.")
...
def excluir_tipo_de_o2_view(request, id):
    ...
    if request.method == 'POST':
        ...
        Historico.objects.create(
            usuario=request.user, descricao=f"""Removeu o Tipo de O2 <span class="badge d-inline-flex align-items-center fw-bolder text-center" style="background-color: {obj.cor};">{obj.nome}</span>.""")

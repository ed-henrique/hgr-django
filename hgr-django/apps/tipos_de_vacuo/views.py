...
def tipos_de_vacuo_view(request):
    ...
    if query:
        ...
        objs = TipoDeVacuo.objects.filter(removido_em__isnull=True).order_by("nome")
...
def excluir_tipo_de_vacuo_view(request, id):
    ...
    if request.method == "POST":
        ...
        Historico.objects.create(
            usuario=request.user,
            descricao=f"""Removeu o Tipo de Vácuo <span class="badge d-inline-flex align-items-center fw-bolder text-center" style="background-color: {obj.cor};">{obj.nome}</span>.""",
        )

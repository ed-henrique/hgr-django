...
class PacienteCreateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if data_de_internacao and hora_de_internacao:
            if data_de_internacao == current_date and hora_de_internacao > current_time:
                self.add_error(
                    "hora_de_internacao", "A hora de internação não pode estar no futuro."
                )
        ...
        if precisa_de_vacuo and not tipo_de_vacuo:
            self.add_error(
                "tipo_de_vacuo", "O tipo de vácuo é obrigatório se o paciente precisar de vácuo.")
        ...
        if precisa_de_cirurgia and not tipo_de_cirurgia:
            self.add_error(
                "tipo_de_cirurgia", "O tipo de cirurgia é obrigatório se o paciente precisar de cirurgia.")
    ...
    leito = forms.ModelChoiceField(
        label="Leito",
        queryset=Leito.objects.filter(removido_em__isnull=True),
        widget=forms.Select(
            attrs={"class": "form-select"},
        ),
        empty_label=None,

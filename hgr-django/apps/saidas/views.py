...
def saidas_view(request):
    ...
    if query:
        ...
        objs = Saida.objects.filter(removido_em__isnull=True).order_by("-data")
    ...
    tipos_de_saida = TipoDeSaida.objects.filter(removido_em__isnull=True).order_by(
        "nome"
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            Saida.objects.create(
                ...
                unidade_de_saude_de_destino=form.cleaned_data[

...
def transferencias_view(request):
    ...
    leitos = Leito.objects.filter(
        removido_em__isnull=True, paciente__isnull=True).order_by('id')
...
def editar_transferencia_view(request, id):
    ...
    pacientes = Paciente.objects.filter(removido_em__isnull=True).exclude(
        saida__isnull=False).order_by('nome')
    leitos = Leito.objects.filter(
        removido_em__isnull=True, paciente__isnull=True).exclude(id=obj.leito_de_origem.id).order_by('id')
